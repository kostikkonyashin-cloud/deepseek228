...
const imagePreview = ref(props.brand.image_path || null)
        ...
        <template #content>
            <div class="bg-white rounded-xl shadow-sm p-6">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-semibold text-gray-800">Редактирование бренда</h3>
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex items-center gap-4">
                            <div v-if="imagePreview" class="w-20 h-20 bg-gray-100 rounded-lg overflow-hidden relative">
                                <img :src="imagePreview" :alt="brand.name" class="w-full h-full object-cover">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-gray-400 mt-1">Оставьте пустым, если не хотите менять логотип</p>
                            </div>
                        </div>
                        <p v-if="form.errors.image" class="mt-1 text-sm text-red-500">{{ form.errors.image }}</p>
                    </div>

                    <!-- Текущий логотип (информация) -->
                    <div v-if="brand.image_path && !imagePreview" class="bg-gray-50 rounded-lg p-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Текущий логотип</label>
                        <div class="flex items-center gap-4">
                            <div
                                class="w-16 h-16 bg-white rounded-lg border border-gray-200 flex items-center justify-center p-2">
                                <img :src="brand.image_path" :alt="brand.name"
                                    class="max-w-full max-h-full object-contain">
                            </div>
                            <span class="text-xs text-gray-500 break-all">{{ brand.image_path }}</span>
                        </div>
                    </div>
